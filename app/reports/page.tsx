...
export default function ReportsPage() {
    ...
    try {
      ...
      if (isDemoUser) {
        ...
        if (memorizationResponse.error) {
          console.error(
            "Error loading memorization:",
            memorizationResponse.error
          );
        } else {
  ...
  const getMemorizationStats = () => {
    ...
    const kurangCount = monthlyData.filter(
      (r) => r.quality === "Kurang"
    ).length;
  ...
  const exportToCSV = () => {
    ...
    attendanceData.forEach((record) => {
      csvContent += `${record.date},${record.ustadzName},${
        record.sabaq ? "Ya" : "Tidak"
      },${record.sabqi ? "Ya" : "Tidak"},${record.manzil ? "Ya" : "Tidak"},"${
        record.notes
      }"\n`;
    });
    ...
    csvContent +=
      "Tanggal,Nama Santri,Halaqoh,Surah,Ayat Dari,Ayat Sampai,Kualitas,Catatan\n";
    ...
    link.setAttribute(
      "download",
      `laporan_${selectedMonth + 1}_${selectedYear}.csv`
    );
            ...
            <div className="mb-8">
              ...
              <div className="bg-gradient-to-r from-purple-500 to-pink-600 rounded-2xl p-8 text-white shadow-xl">
                ...
                <p className="text-purple-100">
                  Rekap kehadiran Ustadz dan hafalan santri
                </p>
              </div>
            </div>
          </FadeIn>
              ...
              },
              {
                ...
                subtitle: `${
                  memorizationStats.totalRecords > 0
                    ? Math.round(
                        (memorizationStats.baikCount /
                          memorizationStats.totalRecords) *
                          100
                      )
                    : 0
                }% dari total`,
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        {
                          label: "Sabaq",
                          count: attendanceStats.sabaqCount,
                          color: "blue",
                        },
                        {
                          label: "Sabqi",
                          count: attendanceStats.sabqiCount,
                          color: "green",
                        },
                        {
                          label: "Manzil",
                          count: attendanceStats.manzilCount,
                          color: "purple",
                        },
                      ].map((item, index) => (
                        <motion.div
                        ...
                        {getUniqueUstadz().length === 0 ? (
                          ...
                          getUniqueUstadz().map((ustadz, index) => (
                            ...
                            >
                              ...
                              >
                                {
                                  getMonthlyAttendance().filter(
                                    (r) => r.ustadzName === ustadz
                                  ).length
                                }{" "}
                                hari
                              </Badge>
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        {
                          label: "Baik",
                          count: memorizationStats.baikCount,
                          color: "green",
                        },
                        {
                          label: "Cukup",
                          count: memorizationStats.cukupCount,
                          color: "yellow",
                        },
                        {
                          label: "Kurang",
                          count: memorizationStats.kurangCount,
                          color: "red",
                        },
                      ].map((item, index) => (
                        <motion.div
                        ...
                        {getUniqueSantri().length === 0 ? (
                          ...
                          getUniqueSantri().map((santri, index) => (
                            ...
                            >
                              ...
                              >
                                {
                                  getMonthlyMemorization().filter(
                                    (r) => r.santriName === santri
                                  ).length
                                }{" "}
                                sesi
                              </Badge>
